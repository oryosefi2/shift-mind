...
export function Schedule() {
  ...
  const loadExistingSchedule = async () => {
    ...
    console.log('Loading existing schedule for week:', currentWeek, 'business:', businessId);
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log('No existing schedule found for this week, status:', response.status);
      }
    } catch (error) {
  ...
  useEffect(() => {
    console.log('useEffect triggered - currentWeek:', currentWeek, 'businessId:', businessId);
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
            ...
            <div>
              {/* כפתור טעינת סידור קיים */}
              <button
                onClick={loadExistingSchedule}
                className="w-full px-4 py-2 mb-2 bg-blue-600 text-white rounded-md font-medium hover:bg-blue-700 transition-colors"
              >
                טען סידור קיים
              </button>
        ...
        <div className="bg-white rounded-lg shadow-md p-4 mb-6">
          <h3 className="text-lg font-semibold mb-2">סטטוס</h3>
          <div className="text-sm">
            <div>שבוע נוכחי: {currentWeek}</div>
            <div>מזהה עסק: {businessId || 'לא זמין'}</div>
            <div>סידור טעון: {scheduleData ? '✅ כן' : '❌ לא'}</div>
            {scheduleData && <div>מזהה סידור: {scheduleData.schedule_id}</div>}
          </div>
        </div>
